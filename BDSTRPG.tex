...
than $1+1/\a$ vertices, then we could add $v$ to that star, contradicting
maximality. (Here we allow either vertex of a 2-vertex star to be
considered the ``centre''). Otherwise, if $v$ has a neighbour $w$
which is a leaf of one of the stars in $Q$, then we could remove
that leaf from its star and create a new 2-vertex star with edge $vw$,
again contradicting maximality. The remaining case is where each of
the (at least $\a n$) neighbours of $v$ is a centre of a star with
$1+\floor{1/\a}>1/\a$ vertices. But these stars would comprise more
than $n$ vertices, which is again a contradiction. We conclude that
$Q$ covers $G$, as desired.
\end{proof}
...
$T_{1},\dots,T_{q}$ of size at most $1/\a'$, as guaranteed by \ref{lem:star-cover}.
...
\]
clusters $V_{i}^{\h}$ which have at least $\delta''s\left(1-\varepsilon'\right)n$
...
inequality, after this procedure a.a.s. at most $2\left(2\varepsilon'n\right)/\left(\left(\a'\right)^{2}/\left(2s\right)\right)\le\left(8\varepsilon'/\left(\a'\right)^{2}\right)\left|W_{i}^{\h}\right|\le\left(9\varepsilon'/\left(\a'\right)^{2}\right)\left|V_{i}^{\h}\right|$
bad vertices have been added to each $V_{i}^{\h}$. By \ref{lem:robust-regular},
for small $\varepsilon'$ each $\left(V_{i}^{1},V_{i}^{2}\right)$
is now $\left(O\left(\sqrt{\varepsilon'}\right),\delta''/2\right)$-super-regular.
With $\delta=\delta''/2$, $a=s/2$ and $b=2s/\a'$, and $\varepsilon'$
small relative to $\varepsilon$, we are done.
\end{proof}
...
to a uniformly random bijection between $X$ and $Y$. Let $X_{\bi}=V_{\bi}\cap X$
and $Y_{\bi}=V_{\bi}\cap Y$, and let $Z_{\bi,\bj}$ be the set of
special pairs in $X_{\bi}\times Y_{\bj}$. Let $X_{\bi,\bj}\subseteq X_{\bi}$
and $Y_{\bi,\bj}\subseteq Y_{\bj}$ be the set of $\x$-type and $\y$-type
vertices in pairs in $Z_{\bi,\bj}$, respectively. Also let $W_{\bi}=V_{\bi}\backslash F$
be the set of ``free'' vertices remaining in $V_{\bi}$.
...
and each $\left|X_{\bi,\bj}\right|=\left|Y_{\bi,\bj}\right|=\left|Z_{\bi,\bj}\right|\sim\psi s_{\bi}s_{\bj}n$.
...
Suppose $\left(V^{1},V^{2}\right)$ is $\left(\varepsilon,\delta\right)$-super-regular,
let $n_{\h}$ satisfy $\left|V^{\h}\right|\ge n_{\h}=\Omega$$\left(n\right)$
(the implied constant can depend on $\varepsilon$ and $\delta$)
and for each $\h$ let $W^{\h}\subseteq V^{\h}$ be a uniformly random
subset of size $n_{\h}$. Then $\left(W^{1},W^{2}\right)$ is a.a.s.
$\left(2\sqrt{\varepsilon},\delta/2\right)$-super-regular.
\end{lem}
To prove \ref{lem:random-preserves-superregular} we will apply a
bipartite version of Szemer\'edi's regularity lemma on the edges
between $V^{1}$ and $V^{2}$. The following formulation can be found
in \cite{Tao05}.
\begin{lem}[Szemer\'edi's regularity lemma, bipartite version]
\label{lem:szemeredi-regularity-bipartite}For any $\varepsilon>0$,
there is $K=K\left(\varepsilon\right)$ such that the following holds.
For any bipartite graph $\G$ with bipartition $V^{1}\cup V^{2}$,
there are partitions of each $V^{\h}$ into clusters $V_{0}^{\h},V_{1}^{\h},\dots,V_{k}^{\h}$
($k\le K$), such that the ``exceptional clusters'' $V_{0}^{\h}$
has size at most $\varepsilon\left|V^{\h}\right|$, the other clusters
$V_{i}^{\h}$ in each part have equal size $s_{\h}\left|V^{\h}\right|$,
and all but $\varepsilon k_{1}k_{2}$ of the pairs of non-exceptional
clusters are $\varepsilon$-regular.
\end{lem}


We will also need a linear relaxation of the notion of an $\left(\varepsilon,\delta\right)$-dense
pair.
\begin{lem}
\label{lem:density-relaxation}If $\left(V^{1},V^{2}\right)$ is $\left(\varepsilon,\delta\right)$-dense
then for any functions $\pi^{\h}:V^{\h}\to\left[0,1\right]$, we have
\[
...
\end{lem}
\begin{proof}
Let $f:\left[0,1\right]^{V^{1}\times V^{2}}\to\RR$ be defined by
\[
...
\]
Note that $f$ is linear in each $\pi^{\h}$ so it attains its extrema
at vertices of the hypercube $\left[0,1\right]^{V^{1}\times V^{2}}$.
But if each $\pi^{\h}\left(v\right)\in\left[0,1\right]$ then $\pi^{1}$
and $\pi^{2}$ just correspond to subsets $U^{\h}\subseteq V^{\h}$.
We just need to show that $f\left(\pi\right)\ge-\varepsilon\delta\left|V^{1}\right|\left|V^{2}\right|$
for each such $\pi$. If each $\left|U^{\h}\right|\ge\varepsilon\left|V^{\h}\right|$
then $\left(\varepsilon,\delta\right)$-density implies $f\left(\pi\right)=\varepsilon\left|U^{1}\right|\left|U^{2}\right|-\delta\left|U^{1}\right|\left|U^{2}\right|\ge0$.
Otherwise $\left|U^{1}\right|\left|U^{2}\right|\le\varepsilon\left|V^{1}\right|\left|V^{2}\right|$
so trivially $f\left(\pi\right)\ge-\varepsilon\delta\left|V^{1}\right|\left|V^{2}\right|$.
\end{proof}

\begin{proof}[Proof of \ref{lem:random-preserves-superregular}]
First, note that for each $v\in W^{\h}$, the number of neighbours
of $v\in W^{\flip{\h}}$ is hypergeometrically distributed, with expected
value at least $\delta n_{\flip{\h}}$. Since $n_{\flip h}=\Omega\left(n\right)$,
a concentration inequality and the union bound immediately tells us
that a.a.s. each such $v$ has $\delta n_{\flip h}/2$ such neighbours.

It remains to show that $\left(W^{1},W^{2}\right)$ is a.a.s. $\left(2\sqrt{\varepsilon},\delta/2\right)$-dense.
Apply \ref{lem:szemeredi-regularity-bipartite} to the bipartite graph
between $V^{1}$ and $V^{2}$, with some regularity parameter $\varepsilon'$
to be determined. By a concentration inequality, a.a.s. for all $i\ge1$
we have $\left|W^{\h}\cap V_{i}^{\h}\right|\le\left(1+\gamma\right)s_{\h}n_{\h}$
for some $\gamma\to0$. Now, consider any $U^{\h}\subseteq W^{\h}$
with $\left|U^{\h}\right|\ge2\sqrt{\varepsilon}n_{\h}$, and let $U_{i}^{\h}=U^{\h}\cap V_{i}^{\h}$.
Let $I^{\h}=\left\{ i\ge1:\left|U_{i}^{\h}\right|\ge\varepsilon'\left|V_{i}^{\h}\right|\right\} $
...
\end{align*}
Now, we define $\pi^{1},\pi^{2}$ as follows. If $v$ is in some exceptional
cluster $V_{0}^{\h}$ then let $\pi^{\h}\left(v\right)=0$. Otherwise
if $v\in V_{i}^{\h}$ let $\pi^{\h}\left(v\right)=\left|U_{i}^{\h}\right|/\left(\left(1+\gamma\right)s_{1}n_{1}\right)\in\left[0,1\right]$.
By \ref{lem:density-relaxation}, 
\begin{align*}
...
\]
Let $\varepsilon'=\varepsilon\delta n_{1}n_{2}/\left(6\left|V^{1}\right|\left|V^{2}\right|\right)$,
so that $e\left(U^{1},U^{2}\right)\ge\left(\delta/2\right)\left|U^{1}\right|\left|U^{2}\right|$
as required.
\end{proof}
...
and $\left(W_{\bi},Y_{\bj,\flip{\bi}}\right)$ are $\left(\varepsilon',\delta'\right)$-super-regular,
where $\varepsilon'=2\sqrt{\varepsilon}$ and $\delta'=\delta/2$.
...
\begin{lem}
\label{lem:adjusting-paths}For any $\delta>0$, $f<1$, $\xi>0$,
and any integer $k>2$ there is $\c=\c\left(\delta,\xi,k\right)$
...
for $\delta_{1}=\delta/2$ each $x\in X$ (respectively, $y\in Y$)
has at least $\delta_{1}\left|W_{1}\right|$ neighbours in $W_{1}$
(respectively, $\delta_{1}\left|V_{k-1}\right|$ neighbours in $W_{k-1}$).
...
is adjacent to $\y^{i}$). By linearity of expectation, $\E\left|I_{X}\right|,\E\left|I_{Y}\right|\ge\left(\delta_{1}/2\right)n$.
If we vary the ordering $v_{1}^{1},\dots,v_{1}^{n}$ (respectively
$v_{k-1}^{1},\dots,v_{k-1}^{n}$) by a transposition, this changes
$\left|I_{X}\right|$ (respectively $\left|I_{Y}\right|$) by at most
2. So, by a bounded-differences concentration inequality (for example
\cite[Lemma~B.1]{FP04}), a.a.s. we have $\left|I_{X}\right|,\left|I_{Y}\right|\ge\left(\delta_{1}/3\right)n$.
...
where $\gamma\left(\delta_{1}\right)=\delta_{1}^{2}/10$. The indices
in $I_{X}\cap I_{Y}$ correspond to $\gamma\left(\delta_{1}\right)n$
special paths $\x^{i},v_{1}^{i},\dots,v_{k-1}^{i},\y^{i}$.

Remove these special paths and and their vertices in $W_{i}$. Let
$\delta_{i}=\delta_{1}/\left(1+\left(i-1\right)/r\right)$; by concentration
and the union bound the situation is still a.a.s. $\delta_{2}$-good
after these removals. We can therefore repeat the previous argument
using $\R_{2}$ to find more special paths. After repeating the argument
all $r$ times (before iteration $i$ we are $\delta_{i}$-good),
there are at most 
\[
n\prod_{i=0}^{r-1}\left(1-\gamma\left(\delta_{i}\right)\right)\le\left(1-\gamma\left(\delta/4\right)\right)^{r}n
\]
vertices remaining in $X$ and in $Y$. If we choose large enough
$r$ this is fewer than $\xi n$ remaining vertices, as required.
\end{proof}
Now we return to the proof of \ref{thm:main-theorem}. For each $\bi,\bj$
with $\bj\ne\flip{\bi}$, and some very small $\xi=\xi\left(a,\delta\right)>0$,
...
applications of \ref{lem:adjusting-paths}. To be precise, as in the
proof of \ref{lem:adjusting-paths}, we split $\R_{2}$ into $\left(2q\right)^{2}$
...
$V_{\bi},W_{\bi},X_{\bi,\bj},Y_{\bi,\bj}$. By \ref{lem:random-preserves-superregular},
each $\left(X_{\bi,\bj},W_{\flip{\bi}}\right)$, $\left(W_{\bi},W_{\flip{\bi}}\right)$
and $\left(W_{\bi},Y_{\bj,\flip{\bi}}\right)$ are $\left(\varepsilon'',\delta''\right)$-super-regular,
where $\varepsilon''=2\sqrt{\varepsilon'}$ and $\delta''=\delta'/2$.
...
in $\G$, where the $w_{\ell}$ alternate between $W_{\bi}$ and $W_{\flip{\bi}}$
...
For large $k$ each $a''n\le\left|X_{\bi}\right|,\left|Y_{\bi}\right|,\left|W_{\bi}\right|\le b''n$
for some $a'',b''>0$, and each $\left|X_{\bi}\right|,\left|Y_{\bi}\right|\le\left|W_{\bj}\right|$.
...
for some $m>0$, where one of these inequalities is an equality. Use
\ref{lem:adjusting-paths} and $\R_{4}$ to find $\floor{m/\left(\left(k-1\right)/2-1\right)}$
...
has $\left(k-1\right)/2-r$ interior vertices in $W_{\bi}$, and has
$r$ interior vertices in $W_{\bj}$. We now have either $\left(k-1\right)\left|X_{\bi}\cup Y_{\bi}\right|/2=\left|W_{\bi}\right|$
or $\left(k-1\right)\left|X_{\bj}\cup Y_{\bj}\right|/2=\left|W_{\bj}\right|$.
...
where $\varepsilon'''=2\sqrt{\varepsilon''}$ and $\delta'''=\delta''/4$.
