...
later (these $\c_{i}$ will in turn determine $\c$).
...
can embed $\T'$ into $\R_{1}$. Let $A'\subseteq V\left(\G\cup\R\right)$
be the set of vertices of $\T'$ which had a leaf deleted from them,
and let $B\subseteq V\left(\G\cup\R\right)$ be the set of $\lambda n$
...
\[
\Pr\left(\left|N_{\R}\left(W\right)\right|\hspace{-1bp}<\hspace{-1bp}\left|W\right|\right)\le\prod_{i=2}^{r}\hspace{-1bp}\Pr\left(\left|N_{i}\cup N_{1}\right|\hspace{-1bp}<\hspace{-1bp}\left|W\right|\right)=\prod_{i=2}^{r}\hspace{-1bp}\Pr\left(\left|N_{i}\cap N_{1}\right|\hspace{-1bp}>\hspace{-1bp}\left|N_{i}\right|\hspace{-1bp}+\hspace{-1bp}\left|N_{1}\right|\hspace{-1bp}-\hspace{-1bp}\left|W\right|\right)=e^{-\Omega\left(r\a^{2}n\right)}.
\]
...
for $\delta''=\delta/2$. Combining the exceptional set $V_{0}$ and
all the $V_{i}^{\h}\backslash W_{i}^{\h}$, there are at most $2\varepsilon'n$
``bad'' vertices not part of a super-regular pair.

Each bad vertex $v$ has at least $\left(\a-2\varepsilon'\right)n$
neighbours to the clusters $V_{i}^{\h}$, and if $\delta''<\a-2\varepsilon'$
then some cluster must have $\delta''s\left(1-\varepsilon'\right)n$
...
\]
clusters which have at least $\delta''s\left(1-\varepsilon'\right)n$
...
inequality, after this procedure a.a.s. at most $2\left(2\varepsilon'n\right)/\left(\a/\left(2s\right)\right)\le\left(8\varepsilon'/\a\right)\left|W_{i}^{\h}\right|\le\left(9\varepsilon'/\a\right)\left|V_{i}^{\h}\right|$
...
With $\varepsilon'$ small relative to $\varepsilon$, we are done.
\end{proof}
...
the following lemma.
\begin{lem}
\label{lem:random-preserves-superregular}Fix any $a>0$ and any $\rho<1$.
Let $\left(V^{1},V^{2}\right)$ be an $\left(\varepsilon,\delta\right)$
super-regular pair in a graph $\G$, where each $\left|V^{\h}\right|\ge n$.
For any $n_{\h}\ge an$, let $W^{\h}$ be a uniformly random $n_{\h}$-vertex
subset of $V^{\h}$. Then $\left(W^{1},W^{2}\right)$ is a.a.s. $\left(\varepsilon,\delta\rho\right)$-super-regular.\end{lem}
\begin{proof}
Since $W^{\h}\subseteq V^{\h}$, by definition $\left(W^{1},W^{2}\right)$
is $\left(\varepsilon,\delta\rho\right)$-dense. Each vertex in $W^{\h}$
...
and the union bound, at least $\delta\left|W^{\flip{\h}}\right|\rho$
...
\begin{lem}
\label{lem:adjusting-paths}For any $\delta>0$, $f<1$, $\rho<1$,
and any integer $k>2$ there is $\c=\c\left(\delta,f,k\right)$ such
...
is now $\left(\varepsilon,\delta\rho^{2}\right)$-super-regular.
\end{lem}
...
Suppose that each $\left|V_{i}\right|=n$, and suppose that in $\G$,
each $v\in V_{0}$ (respectively, $v\in V_{k}$) has at least $\delta n$
neighbours in $V_{2}$ (respectively, in $V_{k-1}$). Suppose further
that each $\x\in X$ is bijectively paired with a vertex $\y\in Y$,
...
By \ref{lem:bipartite-matching}, in $\R$ we can find a matching
of $\left(1-1/\left(2\left(k-2\right)\right)\right)n$ edges between
each $V_{i}$ and $V_{i+1}$ ($1\le i<k-1$). This gives us a disjoint
union of $n/2$ paths $P_{1},\dots,P_{n/2}$; we write $P_{i}=v_{1}^{i}\dots v_{k-1}^{i}$
with $v_{j}^{i}\in V_{j}$. For each $j$, we can assume that $v_{j}^{1},\dots,v_{j}^{n/2}$
is a uniformly random ordering of a uniformly random set of $n/2$
elements of $V_{j},$which we can complete into a uniformly random
ordering $v_{j}^{1},\dots,v_{j}^{n}$ (independently for $1\le j\le k-1$).
...
$\left|S_{X}\right|$ (respectively $\left|S_{Y}\right|$) by at most
2. So, by a bounded-differences concentration inequality (for example
\cite[Lemma~B.1]{FP04}), a.a.s. we have $\left|S_{X}\right|,\left|S_{Y}\right|\ge\delta n/3$.
...
paths $\x^{i},v_{1}^{i},\dots,v_{k-1}^{i},\y^{i}$, where $\varepsilon=\delta^{2}/10$.
\end{proof}

\begin{proof}[Proof of \ref{lem:adjusting-paths}]
For each $V_{i}$, set aside a uniformly random subset $D_{i}$ of
size $\rho\left(\left|V_{i}\right|-t_{i}m\right)$. Divide each $V_{i}\backslash D_{i}$
($1\le i\le k-1$) randomly into $t_{i}$ equal-sized pieces and order
the resulting $k-1$ clusters as $W_{1},\dots,W_{k-1}$ in such a
way that $W_{1}\subseteq V_{1}$ and $W_{k-1}\subseteq V_{r}$. By
\ref{lem:random-preserves-superregular}, $\left(X,W_{1}\right)$
and $\left(W_{k-1},Y\right)$ are $\left(\varepsilon,\delta/2\right)$-super-regular.
Each $\left|V_{i}\backslash D_{i}\right|=\left(1-\rho\right)\left|V_{i}\right|+\rho t_{i}m,$
so each $\left|W_{i}\right|=\Omega\left(n\right)$ and also 
\begin{align*}
m & \le f\left|V_{i}\right|/t_{i}\le\left(f/\left(1-\rho\right)\right)\left|V_{i}\backslash D_{i}\right|/t_{i}-\left(\rho f/\left(1-\rho\right)\right)m,\\
\left(1+\rho f/\left(1-\rho\right)\right)m & \le\left(f/\left(1-\rho\right)\right)\left|V_{i}\backslash D_{i}\right|/t_{i}.
\end{align*}
Let $g=f/\left(\left(1-\rho\right)+\rho f\right)$ so each $g\left|W_{i}\right|\ge m$.
Apply \ref{lem:easy-adjusting-paths} to find $m$ special paths in
$\G\cup\R$ with the required intersection with each $V_{i}$. After
removing these paths, there are $\left|D_{i}\right|/\rho$ vertices
left in each $V_{i}$. If $\left(V_{i},W\right)$ was $\left(\varepsilon,\delta\right)$-super-regular
...
$\left(D_{i},W\right)$ is a.a.s. $\left(\varepsilon,\delta\rho\right)$-super-regular.
So, $\left(V_{i},W\right)$ is now $\left(\varepsilon,\delta\rho^{2}\right)$-super-regular,
as required. By a similar argument, each $\left(V_{i},V_{j}\right)$
that was $\left(\varepsilon,\delta\right)$-super-regular before the
deletions is now $\left(\varepsilon,\delta\rho^{2}\right)$-super-regular
after the deletions.
\end{proof}
...
are $\left(\varepsilon,\delta'\right)$-regular, where $\delta'=\left(\delta/2\right)\rho^{2\left(2q\right)^{2}}=\delta/4$
(for suitable choice of $\rho$). Let $a'=a'\left(a\right)$ satisfy
$\left|W_{\bi}\right|\ge a'n$ (for large $k$, $a'=a^{2}/2$ will
do).
...
where $\delta''=\delta'\rho^{2\left(2q\right)}=\delta'/2$ (for suitable
$\rho$).
