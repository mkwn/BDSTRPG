...
small respective to $\delta_{3}$. Let $a>0$ satisfy $\left|W_{\bi}\right|\ge an$
for all $\bi$.
...
between $W_{\bi}$ and $W_{\flip{\bi}}$ with $w_{k-3}\in W_{\bi}$
(recall, $k$ is odd). Then choose a set $U_{x}\subseteq W_{\flip{\bi}}$
of $\delta_{3}an/2$ neighbours of $w_{k-3}$ and a disjoint set $U_{y}\subseteq W_{\flip{\bj}}$
of $\delta_{3}an/2$ neighbours of $\y$. Now, note that (if $\c_{3}$
is large), there is a.a.s. an edge in $\R_{3}$ between any pair of
disjoint sets of size $\delta_{3}an/2$ (the proof of this is basically
...
is $\left(2\varepsilon_{3},\delta_{3}/2\right)$-super-regular, and
each $\x\in X_{\bi}$ (respectively $\y\in Y_{\bi}$) has at least
$\delta_{3}an/2$ neighbours in $W_{\flip{\bi}}$, throughout the
whole process.
...
and $\left(W_{\bi},Y_{\flip{\bi}}\right)$ is $\left(2\varepsilon_{3},\delta_{3}/2\right)$-regular,
...
For large $k$ each $\left|X_{\bi}\right|,\left|Y_{\bi}\right|,\left|W_{\bi}\right|=\Theta\left(n\right)$,
and each $\left|X_{\bi}\right|,\left|Y_{\bi}\right|\le\left|W_{\bj}\right|$.
...
as desired. During these iterations we have removed uniformly random
subsets of certain sizes from each $X_{\bi}$, $W_{\bi}$ and $Y_{\bi}$,
so by \ref{lem:random-preserves-superregular} each $\left(X_{\bi},W_{\flip{\bi}}\right)$,
$\left(W_{\bi},W_{\flip{\bi}}\right)$ and $\left(W_{\bi},Y_{\flip{\bi}}\right)$
is now $\left(\varepsilon_{4},\delta_{4}\right)$-super-regular, where
$\delta_{4}=\delta_{3}/4$ and $\varepsilon_{4}$ can be arbitrarily
...
is $\left(\varepsilon_{5},\delta_{5}\right)$-super-regular, for $\delta_{5}=\delta_{4}/2$
and $\varepsilon_{5}$ arbitrarily small respective to $\delta_{5}$.
...
``cluster-cycle'' with 3 clusters $S_{\bi}^{*}$, $S_{\bi}^{1}$
and $S_{\bi}^{2}$ of sizes $n_{\bi}$, $\left(k-1\right)n_{\bi}/2$
and $\left(k-1\right)n_{\bi}/2$. This is a $\left(\varepsilon_{5},\delta_{5}\right)$-super-regular
