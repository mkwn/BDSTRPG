...
can embed $\T'$ into $\R_{1}$. Let $A'$ be the set of vertices
of $\T'$ which had a leaf deleted from them, and let $B$ be the
set of $\lambda n$ vertices not part of $\T'$. We can assume $A'$
and $B$ are uniformly random disjoint sets of the appropriate size
(note $\left|A'\right|\ge\lambda n/\D$ and $\left|B\right|=\lambda n$).
For each $b\in B$, note that $N_{\G}\left(b\right)\cap A'$ is hypergeometrically
distributed with expected value at least $\lambda\a n/\D$. Let $\beta=\lambda\a/\left(2\D\right)$;
...
$\beta n$ neighbours in $B$. Similarly each $b\in B$ a.a.s. has
at least $\beta n$ neighbours in $A'$. From now on we treat $A'$
and $B$ as fixed sets satisfying these properties.
...
$M_{i}$ in each $\R_{i}$. Let the random sets $N_{i}=N_{M_{i}}\left(W\right)$
contain the vertices matched with $W$ in each $M_{i}$, and note
that each $\left|W\right|-\a n/3\le\left|N_{i}\right|\le\left|W\right|$.
...
Note that 
\[
\left|N_{i}\right|\left|N_{1}\right|/n\le\left|W\right|^{2}/n\le\left|W\right|\left(1-\a\right)=2\left(\left|W\right|-\a n/3\right)-\left|W\right|-\a n/3\le\left|N_{i}\right|+\left|N_{1}\right|-\left|W\right|-\a n/3,
\]
