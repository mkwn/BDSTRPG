...
Now, embed $\F$ into the random graph $\R_{2}$ (also denote this
embedded subgraph by $\F$). There are $n/\left(2\left(k-1\right)\right)$
...
$B'\subseteq B$ of size $\varepsilon n/2$. We now use a standard
argument also found in the proof of \ref{lem:bipartite-perturbed-stars}.
The probability there is no edge between $A$ and $B$ is less than
the probability there is no edge between $A'$ and $B'$, which is
$\left(1-\c/n\right)^{\left(\varepsilon n/2\right)^{2}}\le e^{-\c\varepsilon^{2}n/2}$.
...
We will take the $\x'$,$\y'$ from the $W_{\bi}^{\left(2\right)}$
and we will take the intermediate vertices in our length-2 paths from
the $W_{\bi}^{\left(1\right)}$. By \ref{lem:random-preserves-superregular},
...
special pair. We claim that if $k$ is large then $\left|N_{\G}\left(\x\right)\cap W_{\flip{\bi}}^{\left(1\right)}\backslash S\right|=\Omega\left(n\right)$
...
is a.a.s. an edge between $N_{\G}\left(\x\right)\cap W_{\flip{\bi}}^{\left(1\right)}\backslash S$
...
and $\left|W_{\br}^{\left(2\right)}\backslash S\right|\le\left|X\right|/\left(2q\right)$.
...
\[
\left|N_{\G}\left(\x\right)\cap W_{\flip{\bi}}^{\left(1\right)}\backslash S\right|\ge\frac{\delta_{2}s_{\flip{\bi}}}{2}n-\left|X_{\bi}\right|\sim\frac{\delta_{2}s_{\flip{\bi}}}{2}n-\frac{2s_{\bi}}{k-1}n\ge\left(\frac{\delta_{2}}{2}-\frac{2\rho}{k-1}\right)s_{\flip{\bi}}n=\Omega\left(n\right),
\]
and
\[
...
random subset of $W_{\bi}^{\left(2\right)}$ of its size. This can
be seen by a symmetry argument, as per \ref{rem:symmetry}. Because
here the situation is a bit complicated, we include some details.
Condition on the random sets $W_{\bi}^{\left(2\right)}$. For each
$\bi$, the vertices in $X_{\bi}'$ are each chosen so that they are
adjacent in $\R_{3}$ to some vertex in one of the $W_{\bj}^{\left(1\right)}$.
Note that the distribution of $\R_{3}$ is invariant under permutations
of $W_{\bi}^{\left(2\right)}$. We can choose such a vertex permutation
$\pi$ uniformly at random; then $\pi\left(X_{\bi}'\right)$ is a
uniformly random subset of $W_{\br}^{\left(2\right)}$ of its size,
which has the right adjacencies in $\pi\left(\R_{3}\right)$ (which
has the same distribution as $\R_{3}$).

Now we claim that even after removing the vertices of our length-2
paths we a.a.s. maintain some important properties about the clusters.
\begin{claim}
\label{claim:move-ok}Let $W_{\bi}'\subseteq W_{\bi}\backslash X_{\bi}'$
be the subset of $W_{\bi}$ that remains after the vertices in the
length-2 paths are deleted. Each $\left(X_{\bi}',W_{\flip{\bi}}'\right)$
...
neighbours in $X_{\bi}'$. Note that $\left|X_{\bi}'\right|\sim2sn/\left(k-1\right)$
and each $s/s_{\bi}\le\rho$, so $\rho\left(k-1\right)\left|X_{\bi}'\right|\ge\left|W_{\flip{\bi}}\right|$
...
Now, for large $k$ note that $\left|W_{\bi}\backslash W_{\bi}'\right|\le\left(\delta_{4}/2\right)\left|W_{\bi}\right|$.
...
\]
For $v\in W_{\bi}$ we have $\left|N_{\G}\left(v\right)\cap W_{\flip{\bi}}\right|\ge\delta_{2}\left|W_{\flip{\bi}}\right|$
by super-regularity, so $\left|N_{\G}\left(v\right)\cap W_{\flip{\bi}}'\right|\ge\left(\delta_{2}/2\right)\left|W_{\flip{\bi}}'\right|$.
...
taken from $W_{\bi}$ (that is, set $X_{\bi}=X_{\bi}'$), and we can
update the $W_{\bi}$ by removing all the vertices used in our length-2
paths (that is, set $W_{\bi}=W_{\bi}'$). All the special pairs are
between some $X_{\bi}$ and $X_{\flip{\bi}}$; let $Z_{\left[\bi\right]}=Z_{\left[\flip{\bi}\right]}$
...
from each $X_{\bi}$ (respectively $W_{\bi}$). Then we can a.a.s.
find special paths in $\G\cup\R_{4}$ with our desired templates in
such a way that after their removal, each $\left(X_{\bi},W_{\flip{\bi}}\right)$
...
of $w_{1}^{j}$ in $X'$, and let $N_{j}^{Y}$ be the vertices of
$Y'$ which form special pairs with the vertices in $N_{j}^{X}$.
Let $N_{j}^{W}$ be the set of neighbours of $N_{j}^{Y}$ in $W_{k-1}$.
If we had $w_{k-1}^{j}\in N_{j}^{W}$ for some $j$, that would mean
there is a special pair $\left(\x,\y\right)\in Z$ with $\x$ adjacent
to $w_{1}^{j}$ and $\y$ adjacent to $w_{k-1}^{j}$; this gives us
a special path $\x w_{1}^{j}\dots w_{k-1}^{j}\y$ as desired.
