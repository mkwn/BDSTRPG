...
$k$-paths. We call such connecting paths ``special paths''. Let
$X\subseteq V$ be the set of special vertices and let $W=V\backslash\F$
be the set of ``free'' vertices. By symmetry we can assume 
\[
...
such that the following holds. (Recall that $\R_{4}\in\GG\left(n,\c_{4}/n\right)$,
that $Q$ is an upper bound on the number of pairs of partner clusters
$q$, and that each $\left(X_{\bi},W_{\flip{\bi}}\right)$ and $\left(W_{\bi},W_{\flip{\bi}}\right)$
are $\left(\varepsilon_{3},\delta_{3}\right)$-super-regular).
...
to choose our desired $\left(1-\xi\right)n$ special paths greedily.
That is, given a collection of fewer than $\left(1-\xi\right)n$ disjoint
special paths running through the clusters, we could find an additional
disjoint special path with the leftover vertices.
...
each of the $w_{1}^{j}$s. Let $N_{j}^{X}$ be the set of neighbours
...
We would like to prove $w_{k-1}^{j}\in N_{j}^{W}$ for some $j$.
This would mean there is a special pair $\left(\x,\y\right)\in Z$
with $\x$ adjacent to $w_{1}^{j}$ and $\y$ adjacent to $w_{k-1}^{j}$;
this gives us a special path $\x w_{1}^{j}\dots w_{k-1}^{j}\y$ as
desired.
...
$2\varepsilon n\le\xi n/2$, so $\left|J'\right|\ge\left|J\right|-\varepsilon n=\left(\xi-2\varepsilon\right)n\ge\xi n/2$.
Now, again using $\left(\varepsilon,\delta\right)$-denseness there
is an edge between each set of $\varepsilon n$ vertices of $Y'$
and each set of $\varepsilon n$ vertices of $W_{k-1}$, so if $j\in J'$
...
from each $W_{\bi}$, so that after removing the special paths with
each template there are will be $l_{\bi}\left|W_{\bi}\right|=\left(1-\sum_{\tau}t_{\bi}^{\tau}\right)\left|W_{\bi}\right|\ge\gamma\left|W_{\bi}\right|$
...
condition on the sizes of the $W_{\bi}'$s and $Z_{i}'$s, each $w\in W_{\flip{\bi}}$
...
arbitrarily. It's a little fiddly to rigorously explain why, so we
wrap up the details in the following claim.
\begin{claim}
...
set of free vertices in our subsets. Note that $\left|Z'\right|=M=\left|X'\right|/2=\left|W'\right|/\left(k-1\right)$.
