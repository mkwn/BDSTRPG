...
$R=R\left(\a\right)$ and $Q=Q\left(\a,\varepsilon\right)$, such
that the following holds. Let $\G$ be a graph on $\range n$ with
minimum degree at least $\a n$. We can choose $q\le Q$ and partition
$\range n$ into clusters $V_{i}^{\h}$ ($1\le i\le q$, $\h=1,2$)
such that each $\left|V_{i}^{\h}\right|=\Theta\left(n\right)$ and
each pair $\left(V_{i}^{1},V_{i}^{2}\right)$ is $\left(\varepsilon,\delta\right)$-super-regular.
Moreover, each $\left|V_{i}^{\h}\right|/\left|V_{j}^{g}\right|\le R$.
\end{lem}
...
and each vertex in $W^{\h}\backslash V^{\h}$ has at least $\delta'\left|W^{\flip{\h}}\right|$
...
is $\left(\varepsilon,\delta\right)$-super-regular. By this super-regularity
each $v\in V^{\h}$ has $\delta\left|V^{\flip{\h}}\right|\ge\delta'\left|W^{\flip{\h}}\right|$
...
of $\varepsilon$. We then have $\left|X_{\bi}\right|\sim2s_{\bi}n/\left(k-1\right)$
...
The choice of ``destination'' $\left\{ W_{\br},W_{\flip{\br}}\right\} $
for each special pair can be almost arbitrary, so long as we distribute
the special pairs roughly evenly between the $W_{\bi}$. An easy way
to do this is to distribute our special pairs randomly. For a special
pair $\left\{ \x,\y\right\} $ with $\x\in X_{\bi}$ and $y\in X_{\bj}$,
uniformly at random choose some $\br$. Let $a=\min_{\bi}\left|W_{\bi}\right|/n$,
so that $\x$ has at least $\delta_{2}an>\left(\delta_{2}a/2\right)n$
neighbours in $W_{\bi}$ and $\y$ has at least $\delta_{2}an>\left(\delta_{2}a/2\right)n$
neighbours in $W_{\bj}$. Now, note that (if $\c_{3}$ is large),
there is a.a.s. an edge in $\R_{3}$ between any pair of disjoint
sets of size $\left(\delta_{2}a/2\right)n$ (the proof of this is
basically identical to the proof of \ref{lem:bipartite-matching}).
Certainly each $\left|W_{\bi}\right|\ge\left(\delta_{2}a/2\right)n$,
so there is an edge between $N\left(\x\right)\cap W_{\flip{\bi}}$
and $W_{\br}$ (respectively, between $N\left(\y\right)\cap W_{\flip{\bj}}$
and $W_{\flip{\br}}$), giving length-2 paths between $\x$ and some
$\x'\in W_{\br}$, and between $\y$ and some $\y'\in W_{\flip{\br}}$.
We can then remove $\x$, $\y$ and the two vertices used in $W_{\flip{\bi}}$
and $W_{\flip{\bj}}$.
...
take more than $\left(\delta_{2}a/2\right)n$ vertices from some $W_{\flip{\bi}}$,
for then we would no longer be able to guarantee that each $\x$,
$\y$ has enough neighbours in the appropriate cluster. But by the
Chernoff bound we will a.a.s. only try to take 
\[
...
of partner clusters). For large $k$ this is fewer than $\left(\delta_{2}a/2\right)n$
vertices. Since the variation in the $s_{\bi}$ is bounded independently
of $\varepsilon$, also $k$ is independent of $\varepsilon$.

Redefine $X_{\bi}$ to be the set of new special vertices taken from
$W_{\bi}$, so that all the special pairs are now between some $X_{\bi}$
...
and let $Z=\bigcup_{i}Z_{i}$ be the set of all special pairs. By
symmetry (considering permutations of $W_{\bi}$ with \ref{rem:symmetry}),
we can assume $X_{\bi}$ is a uniformly random subset of $W_{\bi}$
of its size, so by \ref{lem:random-preserves-superregular} each $\left(X_{\bi},W_{\flip{\bi}}\right)$
is a.a.s. $\left(\varepsilon_{3},\delta_{3}\right)$-super-regular,
where $\delta_{3}=\delta_{2}/2$ and $\varepsilon_{3}$ can be arbitrarily
small respective to $\delta_{3}$.

Also, update $W_{\bi}$ by removing all deleted vertices and all new
special vertices. For large $k$, we are deleting such a small fraction
of each $W_{\bi}$ that each $\left(X_{\bi},W_{\flip{\bi}}\right)$
and $\left(W_{\bi},W_{\flip{\bi}}\right)$ are still $\left(2\varepsilon_{3},\delta_{3}/2\right)$-super-regular.
...
$\left(W_{\bi},W_{\flip{\bi}}\right)$ are still $\left(\varepsilon'',\delta/8\right)$-super-regular.
\end{lem}
...
and also each $\left|W_{\bi}'\right|\ge l_{\bi}\left|W_{\bi}\right|/\left(1+\varepsilon''/2\right)$
and $\left|Z_{i}'\right|\ge r_{i}\left|Z_{i}\right|/\left(1+\varepsilon''/2\right)$
(note that $\xi$ will have to be small relative to $\varepsilon''$
and $\gamma$ for this to be possible). Given these part sizes choose
our partitions uniformly at random.
...
$\left(X_{\bi}',W_{\flip{\bi}}'\right)$ and $\left(W_{\bi}',W_{\flip{\bi}}'\right)$
are a.a.s. $\left(\varepsilon',\delta/2\right)$-super-regular, and
...
After deleting these special paths, what remains in each $W_{\bi}$
(respectively $X_{\bi}$) is a superset of $W_{\bi}'$ with at most
$\left(\varepsilon''/2\right)\left|W_{\bi}'\right|$ extra elements
(respectively, a superset of $X_{\bi}'$ with $\left(\varepsilon''/2\right)\left|X_{\bi}'\right|$
extra elements). This is ensured by the conditions $\left|W_{\bi}'\right|\ge l_{\bi}\left|W_{\bi}\right|/\left(1+\varepsilon''/2\right)$
and $\left|Z_{i}'\right|\ge r_{i}\left|Z_{i}\right|/\left(1+\varepsilon''/2\right)$.
So, if we impose that $\varepsilon'\le\varepsilon''/2$, \ref{lem:robust-regular}
completes the proof.
\end{proof}
